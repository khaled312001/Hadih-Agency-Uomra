            ...
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    <h2 class="h3 mb-1 text-primary">
                        <i class="fas fa-comments me-2"></i>إدارة الرسائل
                    </h2>
                    <p class="text-muted mb-0">إدارة ومتابعة جميع الرسائل في النظام</p>
                </div>
                <div class="d-flex gap-2">
                    <a href="{{ route('admin.messages.create') }}" class="btn btn-primary btn-lg">
            ...
            <div class="col-md-3 mb-3">
                <div class="card bg-info text-white h-100">
                    <div class="card-body text-center">
                        ...
                        <h4 class="mb-1">{{ $messages->pluck('sender_id')->unique()->count() }}</h4>
                        <small>مستخدمين متفاعلين</small>
                    </div>
                </div>
            </div>
        </div>
                        ...
                        <div class="d-flex justify-content-between align-items-center">
                            ...
                            <div class="d-flex gap-2">
                                <select class="form-select form-select-sm" id="filterStatus" onchange="filterMessages()">
                                    ...
                                    <option value="users">من المستخدمين</option>
                                </select>
                            </div>
                        </div>
                    </div>
                                        ...
                                        <tr class="message-row {{ !$message->is_read && $message->receiver_id === auth()->id() ? 'table-warning' : '' }}">
                                            ...
                                            <td class="align-middle">
                                                <div class="d-flex gap-1">
                                                    <a href="{{ route('admin.messages.show', $message) }}" class="btn btn-outline-primary btn-sm">
                                                        <i class="fas fa-eye"></i>
                                                    </a>
                                                    @if($message->sender_id === auth()->id())
                                                        <a href="{{ route('admin.messages.edit', $message) }}" class="btn btn-outline-warning btn-sm">
            ...
            <div class="col-md-8 col-lg-6">
                <div class="card border-0 shadow-sm">
                    <div class="card-body text-center py-5">
                        ...
                        <p class="text-muted mb-4">ابدأ المحادثة مع المستخدمين</p>
                        <a href="{{ route('admin.messages.create') }}" class="btn btn-primary btn-lg">
                            <i class="fas fa-plus me-2"></i>رسالة جديدة
                        </a>
                    </div>
...
function filterMessages() {
    ...
    rows.forEach(row => {
        ...
        if (filter === 'unread') {
            ...
            const senderName = row.querySelector('h6');
            show = senderName && senderName.textContent.includes('إدارة');
